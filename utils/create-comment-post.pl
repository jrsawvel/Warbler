...
my $num_args = $#ARGV + 1;
if ($num_args != 2) {
    print "\nUsage: create-comment-post.pl threadid url\n";
    exit;
}
my $thread_id  = $ARGV[0];
...
my $thread_post_ref = get_thread_post($thread_id);

my $target_url         =   'http://warbler.soupmode.com/thread/' . $thread_id;
...
my $cdb_hash = {
    ...
    # update thread post info based upon the comment added

    $thread_post_ref->{comment_count} = $thread_post_ref->{comment_count} + 1;
    $thread_post_ref->{last_post_date} = $dt;
#    $thread_post_ref->{display_last_post_date} = $formatted_dt;

    update_thread_post($thread_post_ref);
...
sub get_thread_post {
    ...
    my $res = $ua->get( $db_url );

    if ( $res->code > 299 ) {
        die "could not retrieve content for $db_url - code = " . $res->code . " - message = " . $res->message . "\n";
    }

    return decode_json $res->content; # perl hash ref
}

sub update_thread_post {
    my $hash_ref = shift;

    my $uri = 'http://127.0.0.1:5984/warblerdvlp1/' . $hash_ref->{_id};
