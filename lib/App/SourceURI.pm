...
sub process_source_uri {
    my $source_url = shift;
    my $target_url = shift;
    my $web_page   = shift; # content from source url

    my $parent_id = undef;

    my $home_page = Config::get_value_for("home_page");

    my $thread_starter_post = 0;

    if ( $target_url eq Config::get_value_for("thread_post_target_uri") ) {
        $thread_starter_post = 1;
    } elsif ( $target_url =~ m|^$home_page/thread/(\w+)| ) {
            # http://warbler.soupmode.com/thread/XPwZfMYuNaE8AzAEXgfw
            # http://warbler.soupmode.com/thread/XPwZfMYuNaE8AzAEXgfw/our-first-snowfall-may-occur-this-weekend
            $parent_id = $1; 
    } else {
        return {'error' => 'target_not_found', 'error_description' => 'The target URI does not exist.'};  
    }
...
if ( source_url_exists($source_url) ) {
    return {'error' => 'already_registered', 'error_description' => 'The specified WebMention has already been registered.'};  
}
